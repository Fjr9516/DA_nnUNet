...
class SimpleITKDomainIO(BaseReaderWriter):
    # Set "DOMAIN_DICT" as environment variable to the path of the domain dictionary.
    ...
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        ...
        for f in image_fnames:
            if Path(f).name.startswith("BraTS-PED-"):
                domain = 'BraTS-PED'
            elif Path(f).name.startswith("BraTS-GLI-"):
                domain = 'BraTS-GLI'
            else:
                domain = "None"
