...
class nnUNetPredictor(object):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prediction, _ = self.network(x) # JR: prediction = self.network(x)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network(torch.flip(x, (*axes,)))[0], (*axes,)) #JR: prediction += torch.flip(self.network(torch.flip(x, (*axes,))), (*axes,))
